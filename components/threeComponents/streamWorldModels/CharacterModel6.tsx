...
// let moveNum = 0
const CharacterModel6 = ({ scale, rotation }: CharacterModelOpts) => {


    const characterRef = useRef();

    const gltf = useLoader(GLTFLoader, modelList.character6);
    const { nodes, materials, animations } = useGLTF(modelList.character6) as GLTFResult;
    ...
    useEffect(() => {
        ...
        return applyConnectedUser().length > 5 ? (
            <>
                {/* <primitive 
                    ref={characterRef}
                    // onClick={(e) => {
            ...
            <group ref={group}>
                <group  
                    ...
                    ref={characterRef}
                    position={[
                        applyOthersStatus()[4].position.x,
                        applyOthersStatus()[4].position.y,
                        applyOthersStatus()[4].position.z
                    ]}
                    scale={scale} 
                    rotation={[Math.PI / 2, 0, applyOthersStatus()[4].rotateZ]}
    ...
    export default CharacterModel6;
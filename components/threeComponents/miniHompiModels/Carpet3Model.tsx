...
const Carpet3Model = ({rerender, setRerender ,isMyRoom, initFocused}) => {
  const allModelsStatus = useReactiveVar(applyThreeModels);
  const { installed, scale, rotateY, isFocused, position } = allModelsStatus.carpet3[0]


  const createModelStatus = async () => {
    allModelsStatus.carpet3.map(({ position, installed, scale, rotateY }, index) => {
        
        const modelStatus = {
          name: modelNameTypes.carpet3,
          position,
          installed,
    ...
    const gltf = useLoader(GLTFLoader, modelList.carpet_3);
    ...
    const checkFocused = () => {
      const findFocused = allModelsStatus.carpet3.find(model => model.isFocused === true);
      ...
      const findFocusedIndex = () => {
        const findFocusedIndex = allModelsStatus.carpet3.findIndex(model => model.isFocused === true);
    ...
    const installModel = (e) => {
      ...
      if(closedObjPosition && checkFocused() === true && e.target.tagName === "CANVAS"){
          console.log("carpet3Model 포커싱 상태");

          setAllModelsStatus({
            modelName: modelNameTypes.carpet3,
            index: findFocusedIndex(),
            status: {
                ...allModelsStatus.carpet3[findFocusedIndex()],
    ...
    if(installed === true){
        ...
        return (
            ...
            <primitive 
                ...
                {allModelsStatus.carpet3.map((model, index) => {
                  ...
                  if(index > 0 && model.installed === true) {
                      return (
                          ...
                          <primitive 
                              ...
                              position={[
                                  allModelsStatus.carpet3[index].position.x, 
                                  allModelsStatus.carpet3[index].position.y, 
                                  allModelsStatus.carpet3[index].position.z
                              ]} 
                              scale={allModelsStatus.carpet3[index].scale} 
                              rotation={[0, parseFloat(allModelsStatus.carpet3[index].rotateY), 0]}
                              onPointerOver={() => {
                                  document.body.style.cursor = "pointer"
                              }}
                              onPointerOut={() => {

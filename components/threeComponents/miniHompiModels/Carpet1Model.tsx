...
const CarpetModel = ({rerender, setRerender ,isMyRoom, initFocused}) => {
  const allModelsStatus = useReactiveVar(applyThreeModels);
  const { installed, scale, rotateY, isFocused, position } = allModelsStatus.carpet1[0]


  const createModelStatus = async () => {
    allModelsStatus.carpet1.map(({ position, installed, scale, rotateY }, index) => {
        
        const modelStatus = {
          name: modelNameTypes.carpet1,
          position,
          installed,
    ...
    const checkFocused = () => {
      const findFocused = allModelsStatus.carpet1.find(model => model.isFocused === true);
      ...
      const findFocusedIndex = () => {
        const findFocusedIndex = allModelsStatus.carpet1.findIndex(model => model.isFocused === true);
    ...
    const installModel = (e) => {
      ...
      if(closedObjPosition && checkFocused() === true && e.target.tagName === "CANVAS"){
          console.log("carpet1Model 포커싱 상태");

          setAllModelsStatus({
            modelName: modelNameTypes.carpet1,
            index: findFocusedIndex(),
            status: {
                ...allModelsStatus.carpet1[findFocusedIndex()],
    ...
    if(installed === true){
        ...
        return (
            ...
            <primitive 
                ...
                {allModelsStatus.carpet1.map((model, index) => {
                  ...
                  if(index > 0 && model.installed === true) {
                      return (
                          ...
                          <primitive 
                              ...
                              position={[
                                  allModelsStatus.carpet1[index].position.x, 
                                  allModelsStatus.carpet1[index].position.y, 
                                  allModelsStatus.carpet1[index].position.z
                              ]} 
                              scale={allModelsStatus.carpet1[index].scale} 
                              rotation={[0, parseFloat(allModelsStatus.carpet1[index].rotateY), 0]}
                              onPointerOver={() => {
                                  document.body.style.cursor = "pointer"
                              }}
                              onPointerOut={() => {
           ...
           <Indicator 
                position={[
                    allModelsStatus.carpet1[findFocusedIndex()].position.x, 
                    allModelsStatus.carpet1[findFocusedIndex()].position.y +4 * allModelsStatus.carpet1[findFocusedIndex()].scale * 30,
                    allModelsStatus.carpet1[findFocusedIndex()].position.z
                ]} 

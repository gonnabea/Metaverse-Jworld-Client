...
const Carpet2Model = ({rerender, setRerender ,isMyRoom, initFocused}) => {
  const allModelsStatus = useReactiveVar(applyThreeModels);
  const { installed, scale, rotateY, isFocused, position } = allModelsStatus.carpet2[0]


  const createModelStatus = async () => {
    allModelsStatus.carpet2.map(({ position, installed, scale, rotateY }, index) => {
        
        const modelStatus = {
          name: modelNameTypes.carpet2,
          position,
          installed,
    ...
    const checkFocused = () => {
      const findFocused = allModelsStatus.carpet2.find(model => model.isFocused === true);
      ...
      const findFocusedIndex = () => {
        const findFocusedIndex = allModelsStatus.carpet2.findIndex(model => model.isFocused === true);
    ...
    const installModel = (e) => {
      ...
      if(closedObjPosition && checkFocused() === true && e.target.tagName === "CANVAS"){
          console.log("carpet2Model 포커싱 상태");

          setAllModelsStatus({
            modelName: modelNameTypes.carpet2,
            index: findFocusedIndex(),
            status: {
                ...allModelsStatus.carpet2[findFocusedIndex()],
    ...
    if(installed === true){
        ...
        return (
            ...
            <primitive 
                ...
                {allModelsStatus.carpet2.map((model, index) => {
                  ...
                  if(index > 0 && model.installed === true) {
                      return (
                          ...
                          <primitive 
                              ...
                              position={[
                                  allModelsStatus.carpet2[index].position.x, 
                                  allModelsStatus.carpet2[index].position.y, 
                                  allModelsStatus.carpet2[index].position.z
                              ]} 
                              scale={allModelsStatus.carpet2[index].scale} 
                              rotation={[0, parseFloat(allModelsStatus.carpet2[index].rotateY), 0]}
                              onPointerOver={() => {
                                  document.body.style.cursor = "pointer"
                              }}
                              onPointerOut={() => {
           ...
           <Indicator 
                position={[
                    allModelsStatus.carpet2[findFocusedIndex()].position.x, 
                    allModelsStatus.carpet2[findFocusedIndex()].position.y +4 * allModelsStatus.carpet2[findFocusedIndex()].scale * 30,
                    allModelsStatus.carpet2[findFocusedIndex()].position.z
                ]} 

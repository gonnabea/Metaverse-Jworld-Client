...
const Chair2Model = ({rerender, setRerender ,isMyRoom, initFocused}) => {
  ...
  const createModelStatus = async () => {
    ...
    const checkFocused = () => {
      const findFocused = allModelsStatus.chair2.find(model => model.isFocused === true);
      ...
      const findFocusedIndex = () => {
        const findFocusedIndex = allModelsStatus.chair2.findIndex(model => model.isFocused === true);
    ...
    const installModel = (e) => {
      ...
      if(closedObjPosition && checkFocused() === true && e.target.tagName === "CANVAS"){
          console.log("Chair2Model 포커싱 상태");

          setAllModelsStatus({
            modelName: modelNameTypes.chair2,
            index: findFocusedIndex(),
            status: {
                ...allModelsStatus.chair2[findFocusedIndex()],
    ...
    if(installed === true){
        ...
        return (
            ...
            <primitive 
                ...
                {allModelsStatus.chair2.map((model, index) => {
                  ...
                  if(index > 0 && model.installed === true) {
                      return (
                          ...
                          <primitive 
                              ...
                              position={[
                                  allModelsStatus.chair2[index].position.x, 
                                  allModelsStatus.chair2[index].position.y, 
                                  allModelsStatus.chair2[index].position.z
                              ]} 
                              scale={allModelsStatus.chair2[index].scale} 
                              rotation={[0, parseFloat(allModelsStatus.chair2[index].rotateY), 0]}
                              onPointerOver={() => {
                                  document.body.style.cursor = "pointer"
                              }}
                              onPointerOut={() => {

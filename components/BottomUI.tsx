...
const BottomUI = ({ chatContents, ChatForm, CreateRoomForm }:props) => {
    ...
    const ChatBtn = () => {
        return <button onClick={() => {setShowChats(showChats => !showChats); console.log(chatContents)}} onMouseOver={() => playBtnSoundEffect()} className="bg-black text-white rounded-lg hover:bg-blue-500 w-32 h-10 border-double border-4 font-bold z-10" >채팅</button>
    }

    const CreateRoomBtn = () => {
        return <button onClick={() => setShowRoomModal(showRoomModal => !showRoomModal)} onMouseOver={() => playBtnSoundEffect()} className="bg-black rounded-lg text-white hover:bg-blue-500 w-32 h-10 border-double border-4 font-bold z-10" >방 만들기</button>
    }
